    ...
    def __call__(self)->list:
        ...
        if text:
            for row in text:
                ...
                if clip := row.get('ofd:Clips',{}).get('ofd:Clip',[]):
                    if len(clip) and clip[0].get('ofd:Area',{}).get('ofd:Path',{}):
                        cell_d ["clips_pos"] = [float(pos_i) for pos_i in clip[0].get('ofd:Area',{}).get('ofd:Path',{}).get('@Boundary',"").split(" ")]
